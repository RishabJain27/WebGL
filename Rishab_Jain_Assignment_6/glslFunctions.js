...
function create2DTexture(imgPath, magParam, minParam, wrapSParam, wrapTParam, callback) {
  ...
  texImg.onload = function() {
    ...
    if(isPowerOf2(texImg.width) && isPowerOf2(texImg.height)){
...
function sendNormalVertexBufferGLSL(vertexData,dataCount,attribName){
  var data = [];
  for(var i = 0 ; i < vertexData.length; i++) {
...
function sendUniformVec3ToGLSL(val, uniformName) {
  try{
    var canvas = document.getElementById('myCanvas');
    var gl = getWebGLContext(canvas);
    gl.uniform4f(uniformName, val[0], val[1], val[2]);
  }
  catch(ie){
    console.log("error in sendUniformVec4ToGLSL" + ie);
  }
